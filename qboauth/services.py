...
from qboauth.models import Bearer
...
from qboauth import getDiscoveryDocument
...
def getCompanyBalanceSheet(access_token, realmId):
    route = '/v3/company/{0}/reports/BalanceSheet'.format(realmId)
    auth_header = 'Bearer ' + access_token
    headers = {'Authorization' : auth_header,
                'accept' : 'application/json'}
    package = {"date_macro": "This Fiscal Year"}
    r = requests.get(settings.SANDBOX_QBO_BASEURL + route, headers=headers, params=package)
    status_code = r.status_code
    response = r.text
    return response, status_code
